...
try {
  run(['scripts/build-dist.js']);
  assert(process.env.npm_execpath, 'Run this check with npm run test:package');
  const [packed] = JSON.parse(run([process.env.npm_execpath,
  ...
  const dryRun = run([cli, 'install-codex-prompts', '--dry-run'], options);
  ...
  run([cli, 'install-codex-prompts'], options);
  ...
  run([cli, 'install-codex-prompts', '--dry-run', '--force'], options);
  ...
  const declined = run([cli, 'install-codex-prompts'], {
  ...
  run([cli, 'install-codex-prompts', '--force'], options);
  ...
  run([cli, 'init', '--target', consumer, '--non-interactive'], options);
  ...
  run([cli, 'update', '--target', consumer, '--skip-hooks'], options);
  assert.equal(fs.readFileSync(projectGuidance, 'utf8'), 'user-owned project guidance');
