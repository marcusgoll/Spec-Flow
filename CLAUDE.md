...
- UI development: **75-85% faster** with style guide approach (see v3.0.0 below)
...
└── archive/
    ...
    ├── design-principles-legacy-2025-11-07.md (archived v3.0.0)
    └── design-inspirations-legacy-2025-11-07.md (archived v3.0.0)
```
...
## New Features (v3.0.0) - Style Guide Approach

### Overview

**Replaced 3-phase design workflow** (variations → functional → polish) with comprehensive style guide SST approach.

### Why Changed

The multi-phase design workflow was:
- **Too slow**: 2-4 hours per feature (30-75 min per screen × 3 phases)
- **Token-heavy**: 85K tokens per screen
- **Manual bottlenecks**: Human critique gates slowed iteration
- **Low ROI**: 80% of generated variants never used
- **Complex**: 4,675 lines across 5 design commands

### New Approach: Style Guide as SST

**Single source of truth**: `docs/project/style-guide.md` (1,500 lines)

**Core components**:
1. **Foundation** - User's core 9 rules + design philosophy
2. **Colors** - OKLCH system with context-aware token mapping
3. **Typography** - Hierarchy, letter-spacing, line length rules
4. **Spacing & Layout** - 8pt grid system with layout ratios
5. **Visual Elements** - Shadows, gradients, motion, focus states
6. **Components** - Lightweight guidelines (not detailed specs)
7. **Patterns** - Common UI patterns, accessibility, responsive
8. **Implementation** - Tailwind integration, validation rules

### Core 9 Rules (Always Enforced)
...
9. **Squint test**: CTAs and headlines must stand out

### Usage

**UI changes automatically load style guide**:

```bash
/quick "Add student dashboard with cards"
# Detects UI change → loads style-guide.md
# Enforces core 9 rules
# Validates: colors, spacing, typography, accessibility
```

**Style guide generated by /init-project**:
- Pre-populated with project context (brand colors, scale tier)
- Customizable per project
- Version controlled

### Performance Impact

**Before** (3-phase design workflow):
- Time: 2-4 hours per feature
- Tokens: 85K per screen
- Manual gates: 3 (variants, functional, polish)

**After** (style guide approach):
- Time: <30 min per feature
- Tokens: 10-15K per screen
- Manual gates: 0 (automated validation)

**Improvements**:
- **75-85% faster** overall
- **82-88% fewer tokens**
- **Zero manual design critique** (rules enforced automatically)

### Automated Validation

**Checks on every UI change**:
- ✅ No hardcoded colors (hex, rgb, hsl)
- ✅ Spacing on 8pt grid (no arbitrary [Npx])
- ✅ Typography hierarchy (2:1 ratios, correct letter-spacing)
...
**Created**:
- `.spec-flow/templates/style-guide.md` (1,500 lines)
- `docs/STYLE_GUIDE_MIGRATION.md` (migration guide)

**Updated**:
- `/init-project` - Generates 11 docs (added style-guide.md)
- `/quick` - Loads style guide for UI changes, adds validation

**Archived** (moved to `.claude/commands/archive/` and `.spec-flow/memory/archive/`):
- `/design` (orchestrator)
- `/design-variations` (Phase 1)
- `/design-functional` (Phase 2)
- `/design-polish` (Phase 3)
- `/research-design` (inspiration gathering)
- `design-principles.md` (replaced by style guide Foundation section)
- `design-inspirations.md` (replaced by style guide Patterns section)

**Total code reduction**: 5,225 lines removed, replaced with 1,500-line style guide

### Workflow Integration

**Commands that read style guide**:
- `/quick` - For all UI changes
- `/feature` - When UI components involved
- `/init-project` - Generates customized style guide

**Quality gates**:
- Automated validation replaces manual critique
- Style guide rules enforce consistency
- Warnings guide improvements without blocking

### Migration Path

**Automatic** - No user action required:
- Old design commands archived with deprecation notices
- `/quick` automatically detects UI changes and loads style guide
- Existing projects work without changes

**For new UI work**:
```bash
# Old workflow (deprecated)
/design-variations "Dashboard"  # 3-phase process

# New workflow
/quick "Add dashboard with cards"  # Single phase, style-guided
```

**Benefits of migration**:
- Immediate speed improvement (75-85% faster)
- No learning curve (style guide documents existing best practices)
- Better consistency (single SST vs manual decisions)
