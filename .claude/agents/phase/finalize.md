...
- `workflow-state.yaml` - Workflow state for closure
</inputs>
...
- Closes workflow state in workflow-state.yaml
...
WORKFLOW_CLOSED=$(grep -q "status: complete" "workflow-state.yaml" && echo "true" || echo "false")
...
- MUST preserve workflow-state.yaml for audit trail
...
- ✅ `workflow-state.yaml` status set to "complete"
...
**Cause**: workflow-state.yaml not found or corrupted
...
**Recovery**:
1. Verify workflow-state.yaml path
