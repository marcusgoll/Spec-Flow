...
allowed-tools: [Read, Write, Edit, Grep, Glob, Bash(python .spec-flow/*), Bash(git *), Bash(gh *), SlashCommand, TodoWrite, AskUserQuestion]
---
...
- Implement: !`grep -A1 "implement:" specs/*/workflow-state.yaml 2>$null | grep "status:" | awk '{print $2}' || echo "unknown"`
- Optimize: !`grep -A1 "optimize:" specs/*/workflow-state.yaml 2>$null | grep "status:" | awk '{print $2}' || echo "not run"`
</context>
...
**If "status" argument**:
1. Read workflow-state.yaml from current feature directory
2. Display:
   - Deployment model (staging-prod, direct-prod, or local-only)
   - Current phase and status
   - Completed phases (from TodoWrite or workflow-state.yaml)
   - Pending phases
...
3. Read workflow-state.yaml from `$FEATURE_DIR/workflow-state.yaml`
...
1. Read workflow-state.yaml to extract deployment URLs and version
...
1. **Check TodoWrite state**:
   ...
   - If using `/ship status`, verify displayed state matches actual workflow-state.yaml

2. **Verify deployment artifacts**:
   ```bash
   # Check workflow state file
   cat $FEATURE_DIR/workflow-state.yaml | grep -A2 "ship:"
   # Should show: status: completed

   # Check deployment URLs recorded
   cat $FEATURE_DIR/workflow-state.yaml | grep "deployment_url"
...
**Workflow state**:
- `specs/NNN-slug/workflow-state.yaml` - Ship phase marked completed, deployment URLs recorded, version tag recorded
...
**Continue Mode**:
- `/ship continue` resumes from last completed phase
- Reads workflow-state.yaml to find current phase
- Skips completed work, retries failed phase
...
- If continue mode doesn't resume: Check workflow-state.yaml for correct phase status
