...
10. Update state.yaml with version/tag/URLs
...
1. **Never invent production URLs**
   ...
   - Extract actual URLs from state.yaml or deployment logs
   - If URL unknown, say: "Production URL not captured (check deployment platform)"
   - Don't guess at domain names or subdomains

2. **Quote state.yaml exactly for version info**

   - Don't paraphrase deployment metadata
   - If field missing, report: "Version not recorded in state.yaml"
...
5. **Update workflow state**:
   - Writes version, tag, timestamp to state.yaml
...
```bash
Read(specs/$FEATURE_DIR/state.yaml)
```

**Extract production deployment info** from state.yaml:
...
**Format deployment summary** using actual metadata from state.yaml:
...
**All values must come from actual state.yaml content**. Do not invent URLs, versions, or timestamps.
...
5. **Deployment metadata recorded**:
   ...
   - state.yaml updated with production deployment info
...
2. **Verify state.yaml updated**:

   ```bash
   yq eval '.deployment.production.version' "$FEATURE_DIR/state.yaml"
   # Should show actual version number (e.g., "1.2.3")

   yq eval '.deployment.production.tag' "$FEATURE_DIR/state.yaml"
   # Should show tag with v prefix (e.g., "v1.2.3")

   yq eval '.deployment.production.github_release_url' "$FEATURE_DIR/state.yaml"
...
5. **Verify no metadata fabrication**:
   - Production URLs from actual state.yaml (not guessed)
...
- `specs/{feature-slug}/state.yaml` - Updated with:
