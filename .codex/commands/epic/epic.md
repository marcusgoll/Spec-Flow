...
<codex_compatibility>
Codex cannot invoke Task, SlashCommand, Skill, or AskUserQuestion tools. Apply `CODEX_COMPATIBILITY.md`:
- Run the shell blocks directly instead of slash commands.
- Ask clarification/intake questions inline using the same structure.
- Treat subagent/Task delegation as steps you execute sequentially in this session.
- Load referenced skills via `node .codex/skills/test-skill-call.js <name>` when needed.
</codex_compatibility>
...
if (answers["Sprint Estimate"] === "7+ sprints") {
  console.log(
    "⚠️  WARNING: Consider breaking this into multiple smaller epics for better parallelization and faster feedback."
  );
}
```
...
4. Update state.yaml with approval

**If preview auto-skipped:**

- Record in state.yaml: `preview.status: skipped, reason: "Small epic, no UI changes"`

**Commit preview results (atomic commit #6):**

```bash
# Extract metadata for commit message
AUTO_CHECKS=$(yq -r '.preview.auto_checks // "N/A"' epics/${EPIC_SLUG}/state.yaml)
MANUAL_STATUS=$(yq -r '.manual_gates.preview.status // "N/A"' epics/${EPIC_SLUG}/state.yaml)
...
git add epics/${EPIC_SLUG}/state.yaml
...
TodoWrite({
  todos: [
    ...
    {
      content: "Phase 5: Preview (adaptive)",
      status: "pending",
      activeForm: "Preview validation",
    },
    {
      content: "Phase 6: Unified deployment",
      status: "pending",
      activeForm: "Deploying",
    },
    {
      content: "Phase 7: Walkthrough + self-improvement",
      status: "pending",
