...
<codex_compatibility>
Codex cannot invoke AskUserQuestion, SlashCommand, or Task tools. Apply `CODEX_COMPATIBILITY.md`:
- Use the shell blocks here (or `python .spec-flow/scripts/spec-cli.py plan ...`) instead of slash commands.
- Ask clarification questions inline using the same batches/options.
- Treat any “launch sub-agents/Task” instruction as a sequential action you perform in this session.
</codex_compatibility>
