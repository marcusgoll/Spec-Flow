...
<codex_compatibility>
Codex cannot invoke SlashCommand, Task, or AskUserQuestion tools. Apply `CODEX_COMPATIBILITY.md`:
- Run the embedded shell blocks (or `python .spec-flow/scripts/spec-cli.py <phase>`) instead of `/spec`/`/plan` etc.
- When the workflow mentions parallel subagents, execute the steps sequentially in this session.
- Ask any clarifying questions inline rather than via AskUserQuestion.
</codex_compatibility>
